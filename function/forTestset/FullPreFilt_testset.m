% FULLPREFILT_TESTSET 対象の'monkey'の'SharedData'内のデータに対する前処理
% --仕様--
...
for i = 1:recTimes
    ...
    [lfp, BFIL] = test_FiltECoG(LFPOD, LFP_Hz, ResampleRate);
%     [lfp, BFIL] = test_FiltECoG_noButter(LFPOD, LFP_Hz, ResampleRate);
%     [lfp, BFIL] = test_FiltECoG_gausAbs(LFPOD, LFP_Hz, ResampleRate);
%     [lfp, BFIL] = test_FiltECoG_absGaus(LFPOD, LFP_Hz, ResampleRate);
    ...
    elseif InPort(1,1) < 0
        ...
        save(fullfile('ECoG_EMG_Analysis',monkey,'FiltData',...
            [monkey day '_' sprintf('%d',i)]),'EMG_filt',...
            'LFP_filt', '-append')
    end
end
end
