...
% CHECKTIMINGGAP_RECONSTRUCTION 脳波と筋電の遅延時間の計測を目的としたプログラム。
% 結果はVBSRtoolboxを用いた回帰分析と同様に得られるので、相関係数(CC)や、再構成した
% 筋電と計測による筋電の比較から、遅延時間を推定する
...
load([path file],'LFP_filt','EMG_filt','EMG_Hz');
f_name = fieldnames(LFP_filt);
gap = round(0.05*EMG_Hz); % number of frame in 0.05s
ecog = LFP_filt.(f_name{actionPoint})(:,1:end-10*gap,:);
...
for h = 1:10
    gapFront = (h-1)*gap;
    emgSet = emg(:,gapFront+1:end-(11-h)*gap,:);
    for i = 1:K
        A = 0;
        while A ~= N
        ...
        [test(i)] = MakeDataSet(logicalNum_Test{i},TestSize(i),ecog,emgSet,ecogTarget,emgTarget,band);
        [train(i)] = MakeDataSet(logicalNum_Train,TrainSize,ecog,emgSet,ecogTarget,emgTarget,band);
    end
    ...
    mkdir(fullfile('ECoG_EMG_Analysis',monkey,'lookForTimeGap',ZZ));
    addpath(fullfile('ECoG_EMG_Analysis',monkey,'lookForTimeGap',ZZ));
    file = ['emg' sprintf('%03d',5*(h-1)) 'delay'];
    save(fullfile('ECoG_EMG_Analysis',monkey,'lookForTimeGap',ZZ,file),'Result')
